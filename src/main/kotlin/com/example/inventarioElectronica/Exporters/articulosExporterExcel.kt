...
class articulosExporterExcel(private val listaArticulos: List<articulosView>) {
    fun exportar(response: HttpServletResponse) {
        val workbook = XSSFWorkbook()
        val sheet = workbook.createSheet("Articulos")
        ...
        for (art in listaArticulos) {
            ...
            setCell(row,3,art.categoria, dataStyle)
            setCell(row,4,art.marca, dataStyle)
            setCell(row,5,art.estado, dataStyle)
            setCell(row,6,art.plazoGarantia, dataStyle)
            setCell(row,7,art.fechaIngreso, dateStyle)
            setCell(row,8,art.fechaUltimoMantenimiento, dateStyle)
            setCell(row,9,art.fechaProximoMantenimiento, dateStyle)
            setCell(row,10,art.ubicacion, dataStyle)
            row.height = -1
        }
        ...
        response.setHeader("Content-Disposition", "attachment; filename=articulos.xlsx")
